...
// Decomposes queries into 5 subtasks (JSON output), fuses wingman results into final answer
...
// Call Cerebras API with Exponential Backoff (QutieQ Patch 3)
  ...
  for (let attempt = 0; attempt < DORYLUS_CONFIG.maxRetries; attempt++) {
    ...
    try {
      ...
      if (response.status === 429 && attempt < DORYLUS_CONFIG.maxRetries - 1) {
        ...
        console.log(`[DORYLUS] Rate limited (429). Retry ${attempt + 1}/${DORYLUS_CONFIG.maxRetries} in ${delay}ms`);
      ...
      if (!choice?.message?.content) {
        throw new Error('Cerebras API returned empty response');
      }
...
export async function decompose(
  ...
  // Validate key exists
  if (!apiKey) {
    throw new Error('DORYLUS: ALPHA DECOMPOSE API key is empty. Check DORYLUS_KEY_ALPHA_DECOMPOSE in .env');
  }
...
{"subtasks": ["What is the definition of X?", "How does X work technically?", "What are real-world examples of X?", "What are the pros and cons of X?", "What is the current state and future of X?"]}`;
  ...
  try {
    ...
    console.log('[DORYLUS] JSON parse failed, falling back to regex parsing');
...
// FUSE: Take 5 wingman responses → produce one final answer
// QutieQ Patch 4: Context truncation to stay within 8192 token limit
...
export async function fuse(
  ...
  if (!apiKey) {
    throw new Error('DORYLUS: ALPHA FUSE API key is empty. Check DORYLUS_KEY_ALPHA_FUSE in .env');
  }

  // QutieQ Patch 4: Truncate wingman responses to stay within context limit
  // Reserve ~2000 tokens for system prompt + query + fusion overhead
  // Remaining ~4000 tokens split across wingmen = ~800 tokens each
  // Approximate 4 characters per token
  const maxCharsPerWingman = Math.min(1600, Math.floor((DORYLUS_CONFIG.maxContextTokens - 2000) / 5 * 4));
  ...
  const systemPrompt = `CRITICAL CONTEXT: You have a team of research agents who have ALREADY searched the live internet for you. The research reports below contain REAL, CURRENT web data they retrieved moments ago. This is not training data. This is live information from real websites retrieved right now.
...
// Export callCerebras for wingman use
export { callCerebras };
