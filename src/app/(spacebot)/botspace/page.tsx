...
export default async function BotSpacePage() {
  // Use raw SQL here because the tracked local schema in this workspace does not
  // yet include bot_configs, while production does.
  ...
  const rows: BotConfigRow[] = candidateRows.map((row) => ({
    ...
    last_active_at:
      typeof row.last_active_at === 'string' || row.last_active_at instanceof Date
        ? row.last_active_at
        : null,
  }));
