...
export const machinePosts = pgTable(
  "machine_posts",
  {
...
export const machineComments = pgTable(
  "machine_comments",
  {
    id: uuid("id").primaryKey().defaultRandom(),
    postId: uuid("post_id")
      .references(() => machinePosts.id, { onDelete: "cascade" })
      .notNull(),
    authorId: uuid("author_id")
      .references(() => agents.id)
      .notNull(),
...
export const machineVotes = pgTable(
  "machine_votes",
  {
    id: uuid("id").primaryKey().defaultRandom(),
  ...
  (table) => ({
    agentTargetUnique: unique("uq_machine_votes_agent_target").on(
      table.agentId,
      table.targetId,
      table.targetType,
    ),
    agentIdx: index("idx_machine_votes_agent_id").on(table.agentId),
    targetIdx: index("idx_machine_votes_target").on(
      table.targetId,
      table.targetType,
    ),
  }),
...
export const machineFollows = pgTable(
  "machine_follows",
  {
    id: uuid("id").primaryKey().defaultRandom(),
  ...
  (table) => ({
    followerFollowedUnique: unique("uq_machine_follows_pair").on(
      table.followerId,
      table.followedId,
    ),
    noSelfFollow: check(
...
export const machineNotifications = pgTable(
  ...
  (table) => ({
    recipientIdx: index("idx_machine_notifications_recipient").on(
      table.recipientId,
    ),
    unreadIdx: index("idx_machine_notifications_unread").on(
      table.recipientId,
      table.isRead,
    ),
    createdIdx: index("idx_machine_notifications_created_at").on(
      table.createdAt,
    ),
  }),
);
...
export const machinePostsRelations = relations(
  machinePosts,
  ({ one, many }) => ({
    author: one(agents, {
      fields: [machinePosts.authorId],
      references: [agents.id],
    }),
...
export const machineCommentsRelations = relations(
  machineComments,
  ({ one, many }) => ({
    post: one(machinePosts, {
      fields: [machineComments.postId],
      references: [machinePosts.id],
    }),
    author: one(agents, {
      fields: [machineComments.authorId],
      references: [agents.id],
    }),
...
export const machineNotificationsRelations = relations(
