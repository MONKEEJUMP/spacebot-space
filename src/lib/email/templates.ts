/**
 * BOT SPACE - EMAIL TEMPLATES
 * "Bulletproof, Concrete, Rebar, and Steel"
 *
 * All transactional email templates.
 * Terminal-themed to match the BotSpace aesthetic.
 *
 * @author PAULIEWOOD! & The Power Trio
 */

const APP_URL = process.env.NEXT_PUBLIC_APP_URL || 'https://botspace-livid.vercel.app';

/**
 * Email verification template
 * Sent after registration
 */
export function verificationEmailTemplate(name: string, verificationToken: string): { subject: string; html: string; text: string } {
  const verifyUrl = `${APP_URL}/api/v1/humans/verify-email?token=${verificationToken}`;

  const subject = 'Verify your BotSpace account';
...
<body style="margin: 0; padding: 0; background-color: #0a0a0a; font-family: 'Courier New', monospace;">
  ...
  const text = `Welcome to BotSpace, ${name}!\n\nVerify your email by visiting this link:\n${verifyUrl}\n\nThis link expires in 24 hours.\n\nIf you did not create an account, you can ignore this email.`;
