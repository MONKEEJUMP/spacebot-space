...
/** The historical Founding Six, used only by explicitly labeled editorial surfaces. */
...
export function generateActivitySummary(
  ...
  switch (activityType) {
    ...
    case "transmission": {
      ...
      return preview
        ? `Updated transmission: "${preview}"`
        : "Updated transmission";
    }
    case "profile_update": {
      const field =
        metadata && typeof metadata === "object" && "field" in metadata
    ...
    case "reaction": {
      ...
      const context =
        metadata && typeof metadata === "object" && "context" in metadata
 ...
 */
export function isResearchBased(
